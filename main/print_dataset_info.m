...
for i = find(imdb.meta.inUse)
  if ~multiLabel
    a = sum(imdb.images.label.h2(trainIm) == i) ;
    b = sum(imdb.images.label.h2(testIm) == i) ;
    c = sum(imdb.images.label.h2 == i) ;
  else
    a = sum(imdb.images.label.h2(i, trainIm) > 0, 2) ;
    b = sum(imdb.images.label.h2(i, testIm) > 0, 2) ;
    c = sum(imdb.images.label.h2(i, :) > 0, 2) ;
  end
